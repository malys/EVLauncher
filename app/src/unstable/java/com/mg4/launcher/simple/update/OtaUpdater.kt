 ...
 *
 * Security posture is the one shared across the MG4 suite (MG4Control / MG4Tasker):
 ...
 *
 * This replaces the previous updater, which fetched a version manifest from a single
 * third-party host and trusted a SHA-256 published by that same host — an integrity check
 * against a corrupt transfer, not against a compromised server.
 */
object OtaUpdater {

    private const val TAG = "OtaUpdater"
    private const val CACHE_PREFIX = "MG4SimpleLauncher-ota-"
    ...
    /**
     * Numeric core of a version: "1.4.42-unstable" → [1,4,42]. A segment with no digits
    ...
    /**
     * Version carried by an unstable asset name: "MG4SimpleLauncher-unstable-1.4.42.apk" →
     * "1.4.42". The release tag is the fixed string "unstable" (one rolling pre-release), so
     * the asset name is what identifies a build. Null when the name carries no version.
     */
    ...
    fun check(currentVersion: String): Update? {
        ...
        return try {
            conn = (URL(RELEASES_API).openConnection() as HttpURLConnection).apply {
                ...
                setRequestProperty("User-Agent", "MG4SimpleLauncher-Android")
    ...
    fun downloadFileName(versionName: String?): String {
        ...
        return "MG4SimpleLauncher-unstable-$safe.apk"
    }
        ...
        try {
            repeat(6) {
                ...
                val connection = (current.openConnection() as HttpURLConnection).apply {
                    instanceFollowRedirects = false; connectTimeout = TIMEOUT_MS; readTimeout = TIMEOUT_MS
                }
                try {
                    val status = connection.responseCode
                    if (status in 300..399) {
                        current = current.toURI().resolve(connection.getHeaderField("Location") ?: return null).toURL()
                        if (!isAllowedUrl(current.toString())) return null
                    } else {
                        if (status != HttpURLConnection.HTTP_OK) return null
                        FileOutputStream(temporary).use { output ->
                            connection.inputStream.use { it.copyTo(output) }; output.fd.sync()
                        }
            ...
            return null
        } catch (e: Exception) { Log.w(TAG, "Update download failed", e); return null }
    ...
    fun install(context: Context, apk: File): Boolean {
        ...
        } catch (e: Exception) { Log.w(TAG, "Update install failed", e); false }
    }
